...
metrics_dir = os.path.join(current_dir, 'metrics', 'higher_order_functions_metrics')

# Find the raw metrics file
try:
    all_files = os.listdir(metrics_dir)
    raw_metrics_files = [f for f in all_files if 'raw_metrics_after.txt' in f.strip()]
    
    if not raw_metrics_files:
        print("Could not find raw_metrics_after.txt file!")
        exit(1)
        
    raw_metrics_file = os.path.join(metrics_dir, raw_metrics_files[0])
    print(f"Found raw metrics file: {raw_metrics_file}")
    ...
    output_file = os.path.join(metrics_dir, 'raw_averages_after.txt')
    with open(output_file, 'w') as f:
        f.write(f"Average LOC: {statistics.mean(non_zero_loc) if non_zero_loc else 'N/A'}\n")
        f.write(f"Average LLOC: {statistics.mean(non_zero_lloc) if non_zero_lloc else 'N/A'}\n")
        f.write(f"Average SLOC: {statistics.mean(non_zero_sloc) if non_zero_sloc else 'N/A'}\n")
        f.write(f"Average Comments: {statistics.mean(non_zero_comments) if non_zero_comments else 'N/A'}\n")
        f.write(f"Average Multi: {statistics.mean(non_zero_multi) if non_zero_multi else 'N/A'}\n")
        f.write(f"Average Blank: {statistics.mean(non_zero_blank) if non_zero_blank else 'N/A'}\n")
        f.write(f"Average Single comments: {statistics.mean(non_zero_single_comments) if non_zero_single_comments else 'N/A'}\n")
        
        if non_zero_sloc and non_zero_comments:
            f.write(f"Average Comment Density: {statistics.mean(comment_density)}\n")
        else:
            f.write("Average Comment Density: N/A\n")
            
        f.write(f"Total Files Analyzed: {total_files}\n")
        f.write(f"Files With Code: {files_with_code}\n")
        f.write(f"Total LOC: {sum(loc)}\n")
        f.write(f"Total LLOC: {sum(lloc)}\n")
        f.write(f"Total SLOC: {sum(sloc)}\n")
        f.write(f"Total Comments: {sum(comments)}\n")
        f.write(f"Total Blank Lines: {sum(blank)}\n")
